...
public class TigerClient {
    ...
    //TODO: Refactor all this to the new "defensive" strategy
    public static void main(String[] args) {
        if (args.length != 2) {
            System.err.println("Usage: java TigerClient <host name> <port number>");
            System.exit(1);
        }
        ...
        try {
            SocketReaderWriter readerWriter = new SocketReaderWriter(hostName, portNumber);
            BufferedReader stdIn = new BufferedReader(new InputStreamReader(System.in));

            String fromServer;
            TigerClientProtocols client = new TigerClientProtocols(readerWriter);

            while ((fromServer = readerWriter.readLine()) != null) {
                ...
                while ((numOfRounds = client.challengeProtocol()) != -1) {
                    for (int ii = 1; ii <= numOfRounds; ii++) {
                        String opponentPID = client.matchStartProtocol(); // NOT USED FOR NOW

                        //TODO: MultiThread this AKA Dave's suggestion
                        Game game1 = new Game();
                        Game game2 = new Game();
                        Player p1 = new Player(Integer.parseInt(ourPlayerID), game1);
                        Player p2 = new Player(Integer.parseInt(ourPlayerID), game2);
                        ...
                        while ((gid = client.matchEndProtocol()) == Integer.MIN_VALUE) {
                            // Wait for the next move regarding a game
                        ...
                        int otherEndedGameID = client.matchEndProtocol();
                        // Now, gid and otherEndedGameID have both ended matches. Ideally we need the score info too?
                    }
                }
            }
        ...
        try (
            ...
            String fromServer;
            TigerClientProtocols client = new TigerClientProtocols(this);
